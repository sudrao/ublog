  ...
  module MobileFu
    ...
    module ClassMethods
      ...
      def has_mobile_fu(test_mode = false)
        ...
        helper_method :user_agent_device_name
      end
    ...
    module InstanceMethods
      ...
      def set_mobile_format
        if (request.format == Mime::HTML) || !(request.path_parameters['format'])
          if is_mobile_device?
            request.format = session[:mobile_view] == 0 ? :html : :mobile
          else
            request.format = session[:mobile_view] == 1 ? :mobile : :html
          end
          @@in_mobile_view = in_mobile_view?
        end
      end
      ...
      def in_mobile_view?
        if (session[:mobile_view])
          ...
          request.format.to_sym.to_s == "mobile"
        end
      end
